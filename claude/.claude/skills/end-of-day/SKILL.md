...
If `Tracker: none`, this command degrades to: commit summary, threads update, Hemingway bridge draft, AI block refresh. No comments, no new tickets. The bridge + carryover loop is still the point.
...
Reply `go` to apply everything: tracker comments, new tickets, AI block, and (optionally) CONTEXT.md edits. Or selectively: `apply 1,3`, `skip 2`, `change 2 to <KEY>`. The Hemingway bridge draft is yours to paste manually into the `## 🔖 Hemingway bridge` section.
```
...
If a step fails: stop, report, don't proceed. User can re-issue `go` after fixing.
